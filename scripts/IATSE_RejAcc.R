...
for (n in n_samples) {
  ...
  for (mc in 1:MC_size) {
    ...
    X_array[,mc] <- IATSE(t(X_male[, cols]), t(X_female[, cols]))
    Y_array[,mc] <- IATSE(t(Y_male[, cols]), t(Y_female[, cols]))
    Z_array[,mc] <- IATSE(t(Z_male[, cols]), t(Z_female[, cols]))
    
    
    col_idx <- col_idx + n
  }
  
  saveRDS(X_array, paste0("outputs/TestResults/X_IATSE_n_", n, ".rds"), compress = "xz")
  saveRDS(Y_array, paste0("outputs/TestResults/Y_IATSE_n_", n, ".rds"), compress = "xz")
  saveRDS(Z_array, paste0("outputs/TestResults/Z_IATSE_n_", n, ".rds"), compress = "xz")
